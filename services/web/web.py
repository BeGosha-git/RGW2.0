...
def register_robot_endpoints():
    ...
    @flask_app.route('/api/robot/g1/arm_actions', methods=['GET'])
    ...
    @flask_app.route('/api/robot/g1/arm_actions/execute', methods=['POST'])
...
<body>
    ...
    try:
        ...
        if "Permission denied" in error_msg and port < 1024:
            ...
            print(f"Port {port} is already in use. Trying to reclaim configured port...", flush=True)
            try:
                import subprocess
                subprocess.run(
                    ["fuser", "-k", f"{port}/tcp"],
                ...
                if check_port_available(port):
                    print(f"Port {port} reclaimed. Retrying on configured port...", flush=True)
                    run_web_server(port=port, build_dir=build_dir)
                    return
            except Exception:
                pass

            print(f"Could not reclaim port {port}. Trying alternative ports...", flush=True)
            found_port = None
            for try_port in [8080, 8081, 8082, 8083, 8084, 8085]:
